import audios from '../models/audio.models.js';
...
const uploadAudio = async (audio, empid) => {
    ...
    try {
        ...
        const uploadAudioOnCloudinary = await cloudinary.uploader.upload(audio.path, {
            folder: 'audios',
            resource_type: 'video'
        });
...
const getRecentAudio = async (empid,limit = 6) => {
    try {
      const recentAudios = await audios.find({ empid })
        .sort({ uploadedAt: -1 }) 
        .limit(limit); 
      console.log(recentAudios);
      return recentAudios;
    } catch (error) {
      console.error('Error retrieving recent audios from the database:', error.message);
