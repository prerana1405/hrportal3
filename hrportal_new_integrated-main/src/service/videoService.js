...
import cloudinary from '../utils/cloudinary.js';
 ...
 const uploadVideo = async (empid,video) => {
    ...
    try {
        ...
        const uploadVideoOnCloudinary = await cloudinary.uploader.upload(video.path, {
            folder: 'videos',
            resource_type: 'video'
        });
...
const getRecentVideo = async (empid,limit = 6) => {
    try {
      const recentVideos = await videos.find({ empid })
        .sort({ uploadedAt: -1 }) 
        .limit(limit); 
      console.log(recentVideos);
      return recentVideos;
    } catch (error) {
      console.error('Error retrieving recent videos from the database:', error.message);
 ...
 const deleteVideo = async (_id) => {
    ...
    if (!_id) {
       ...
       const deleteVideoFromCloudinary = await cloudinary.uploader.destroy(video.cloudinary_id);
