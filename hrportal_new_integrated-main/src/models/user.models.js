...
const signupUser = (fname, lname, email, empid, password, mobile_no) => {
    return new Promise((resolve, reject) => {
        ...
        bcrypt.hash(password, 10, (error, hashedPassword) => {
            ...
            const query = "INSERT INTO users (fname, lname, email, empid, password, mobile_no, email_token) VALUES (?, ?, ?, ?, ?, ?, ?)";
            connection.query(query, [fname, lname, email, empid, hashedPassword, mobile_no, email_token], (error, result) => {
                if (error) {
                    if (error.code === 'ER_DUP_ENTRY') {
                        console.error("Duplicate entry error:", error.message);
                        reject(new Error('Duplicate entry: a user with the provided email, empid, or mobile_no already exists.'));
                    } else {
                        console.error("Error while executing query:", error);
                        reject(error);
                    }
...
const updateUserVerification = (id) => {
    return new Promise((resolve, reject) => {
        const query = "UPDATE users SET email_token = NULL, is_verified = true WHERE id = ?";
        connection.query(query, [id], (error, result) => {
            if (error) {
...
const updateUserDetails = (id, fname, lname, mobile_no) => {
    return new Promise((resolve, reject) => {
        const query = "UPDATE users SET fname = ?, lname = ?, mobile_no = ? WHERE id = ?";
